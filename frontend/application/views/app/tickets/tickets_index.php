    ...
    <div class="col-md-12">
      <div class="card">
        <div class="card-header">
          <h5 class="title">All Tickets - Helpdesk</h5>
          <hr>
        </div>
        <div class="card-body">
          <div id="tickets">
            ...
            <span v-else>
              <div class="row">
                  ...
                  <div class="table-responsive">
                    <div v-if = "ticketsCount > 0">
                      <table class="table">
                        ...
                        <tbody>
                          <span v-if="showTableLoading === 'yes'">
                            <h4>Loading....</h4>
                          </span>
                          <span v-else>
                          <tr v-for="(item, index) in tickets" :class="{ 'escalated-row': item.escalated === 1 }">
                            <td>{{item.display_ticket_id}}</td>
                            <td>
                              <a :href="'tickets/details/'+item.project_id+'/'+item.id"><font color="black"><u>{{item.heading}}</u></font></a>
                            </td>
                      ...
                      <span>{{ currentPage }} / {{ Math.ceil(ticketsCount / perPage) }}</span>
                      <button @click="onPageChange(currentPage + 1)" :disabled="currentPage * perPage >= ticketsCount">Next</button>
                    </div>
                    <div v-else>
                ...
                <div class="col-md-3">
                  ...
                  <br>
                  <select class="form-control" v-model="projectId" @change="statusProject($event)">
                    <option value="0" :selected="projectId === 0" :disabled="projectId === 0">Choose project</option>
